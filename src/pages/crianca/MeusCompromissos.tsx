...
export default function MeusCompromissos() {
  ...
  // Fetch all compromissos for this child
  ...
  const next15Days = useMemo(() => {
    ...
    const pending = (compromissos ?? []).filter((c) => {
      if (c.concluido) return false;
      const dt = parseISO(c.data_hora);
      return dt < today; // overdue
    });
  ...
  const selectedDayCompromissos = useMemo(() => {
    return (compromissos ?? []).filter((c) =>
      isSameDay(parseISO(c.data_hora), selectedDate)
    );
  }, [compromissos, selectedDate]);

  // Days with compromissos for calendar dots
  const daysWithCompromissos = useMemo(() => {
    return (compromissos ?? []).map((c) => parseISO(c.data_hora));
  }, [compromissos]);
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Provas, consultas, compromissos pessoais</p>
          </div>
              ...
              <>
                {next15Days.pending.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="text-sm font-semibold text-destructive flex items-center gap-1">⚠️ Atrasados</h3>
                    {next15Days.pending.map((c, i) => renderCompromissoCard(c, i))}
                  </div>
                )}
                ...
                {next15Days.upcoming.length > 0 ? (
                  <div className="space-y-2">
                    {next15Days.pending.length > 0 && (
                      <h3 className="text-sm font-semibold text-foreground mt-4">Próximos</h3>
                    )}
                    {next15Days.upcoming.map((c, i) => renderCompromissoCard(c, i))}
                  </div>
                ) : next15Days.pending.length === 0 ? (
            ...
            <Card>
              <CardContent className="p-2">
                <Calendar
                  ...
                  modifiers={{
                    hasCompromisso: daysWithCompromissos,
                  }}
                  modifiersStyles={{
                    hasCompromisso: {
                      fontWeight: "bold",
